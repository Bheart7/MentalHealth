...
df2 = df[['age_bins','self_employed','treatment','family_history']]
...
def update(xaxis_name):
      ...
      'data':[go.Bar(
                    x=pd.crosstab(index=df2['age_bins'], columns=df2[xaxis_name]).index,
                    y=pd.crosstab(index=df2['age_bins'], columns=df2[xaxis_name])['Yes'],
                    name='Yes',
                    marker={'color':'#FFD700'}),
            go.Bar(
                    x=pd.crosstab(index=df2['age_bins'], columns=df2[xaxis_name]).index,
                    y=pd.crosstab(index=df2['age_bins'], columns=df2[xaxis_name])['No'],
                    name='No',
                    marker={'color':'#FF69B4'})],
